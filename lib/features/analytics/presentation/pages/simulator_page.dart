...
class _SimulatorForm extends StatefulWidget {
  final String vehicleId;
  final String vehicleName;
  final double vehiclePrice;

  const _SimulatorForm({
    required this.vehicleId,
    required this.vehicleName,
    required this.vehiclePrice,
  });
  ...
  @override
  State<_SimulatorForm> createState() => _SimulatorFormState();
}

class _SimulatorFormState extends State<_SimulatorForm> {
  ...
  double get _downPaymentAmount => widget.vehiclePrice * (_initialPaymentPct / 100);
  double get _financedAmount => widget.vehiclePrice * (1 - _initialPaymentPct / 100);
    ...
    return BlocListener<SimulatorBloc, SimulatorState>(
      listener: (context, state) {
        if (state.status == Status.success && state.simulation != null) {
          context.push(RouteNames.simulatorSchedule, extra: state.simulation);
        }
                          ...
                          children: [
                            Row(
                              children: [
                                Expanded(
                                  child: TextFormField(
                                    initialValue: _initialPaymentPct.toStringAsFixed(0),
                                    keyboardType: const TextInputType.numberWithOptions(decimal: true),
                                    decoration: _inputDecoration('Cuota Inicial (%)'),
                                    onChanged: (v) {
                                      final parsed = double.tryParse(v);
                                      if (parsed != null) setState(() => _initialPaymentPct = parsed);
                                    },
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: TextFormField(
                                    ...
                                    controller: TextEditingController(
                                      text: '\$${_downPaymentAmount.toStringAsFixed(2)}',
                                    ),
                                    decoration: _inputDecoration('Monto (USD)'),
                                  ),
                                ),
                              ],
                            ),
                            ...
                            TextFormField(
                              readOnly: true,
                              controller: TextEditingController(
                                text: '\$${_financedAmount.toStringAsFixed(2)}',
                              ),
                              decoration: _inputDecoration('Monto a Financiar (USD)'),
                            ),
                            ...
                            const SizedBox(height: 20),
                            TextFormField(
                              initialValue: _teaPercentage.toStringAsFixed(2),
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
                              ...
                              onChanged: (v) {
                                final parsed = double.tryParse(v);
                                if (parsed != null) setState(() => _teaPercentage = parsed);
                              },
                            ),
                            const SizedBox(height: 20),
                            CreditConfigSection(
                              ...
                              onBalloonChanged: (v) {
                                final parsed = double.tryParse(v);
                                if (parsed != null) setState(() => _balloonPaymentPct = parsed);
                              },
                            ),
                            const SizedBox(height: 20),
                            Row(
                              ...
                              children: [
                                ...
                                Switch(
                                  ...
                                  onChanged: (v) => setState(() => _vehicularInsuranceEnabled = v),
                                ),
                              ],
                            ),
                            if (_vehicularInsuranceEnabled) ...[
                              ...
                              DropdownButtonFormField<String>(
                                ...
                                onChanged: (label) {
                                  if (label != null) {
                                    setState(() => _vehicularInsuranceRate = _insuranceRates[label]!);
                                  }
                                },
                              ),
                            ],
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                Switch(
                                  ...
                                  activeThumbColor: AppColors.accent,
                                  onChanged: (v) => setState(() => _desgravamenEnabled = v),
                                ),
                              ],
                            ),
                            ...
                            GracePeriodSection(
                              ...
                              onEnabledChanged: (v) => setState(() => _gracePeriodEnabled = v),
                              onTypeChanged: (v) {
                                if (v != null) setState(() => _gracePeriodType = v);
                              },
                              onMonthsChanged: (v) {
                                final parsed = int.tryParse(v);
                                if (parsed != null) setState(() => _gracePeriodMonths = parsed);
                              },
                            ),
                            const SizedBox(height: 16),
                            GestureDetector(
                              ...
                              child: AbsorbPointer(
                                child: TextFormField(
                                  controller: TextEditingController(
                                    text: DateFormat('dd/MM/yyyy').format(_loanStartDate),
                                  ),
                                  decoration: _inputDecoration('Fecha de Inicio del Préstamo').copyWith(
